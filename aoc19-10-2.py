...
def shoot(field, startX, startY, x, y):    
    dX = x - startX
    dY = y - startY
    ...
    print("shoot sX=", startX, "sY=", startY, "x=", x, "y=", y, "dX=", dX, "dY=", dY)

    i = 0
    while True:
        startX += dX
        startY += dY
        #print("  check field startX=", startX, "startY=", startY)
        if len(field) > startY and len(field[0]) > startX and startY >= 0 and startX >= 0:
            #print("  check field startX=", startX, "startY=", startY)
            if field[startY][startX] == 1:
                field[startY][startX] = 2
                #print("    hit startX=", startX, "startY=", startY)
                return [startX, startY]
        if startY < 0 or startX < 0 or startY > len(field) or startX > len(field[0]):
            return [-1, -1]
        i += 1

def vaporize(field, startX, startY, stopCount=-1):
    tX = startX
    tY = 0
    c = 0
    e = 0
    ...
    assert field[startY][startX] == 1, "no asteroid at %i %i" % (startX, startY)
    field[startY][startX] = 7

    while True:
        e = c
        print(".")
        for x in range (tX, len(field[0]) - b):         # right
            pos = shoot(field, startX, startY, x, 0 + b)
            if pos[0] > -1:
                c += 1
                print("c=", c, "pos=", pos)
                if c > stopCount:
                    return pos
        for y in range (0 + b, len(field) - b):            # down
            pos = shoot(field, startX, startY, len(field[0]) - 1 - b, y)
            if pos[0] > -1:
                c += 1
                print("c=", c, "pos=", pos)
                if c > stopCount:
                    return pos
        for x in range (0 + b, len(field[0]) - b):          # left
            pos = shoot(field, startX, startY, len(field) - x - b, len(field[0]) - 1 - b)
            if pos[0] > -1:
                c += 1
                print("c=", c, "pos=", pos)
                if c > stopCount:
                    return pos
        for y in range (0 + b, len(field) - b):                 # up
            pos = shoot(field, startX, startY, 0 + b, len(field) - y - b)
            if pos[0] > -1:
                c += 1
                print("c=", c, "pos=", pos)
                if c > stopCount:
                    return pos            
        for x in range (0 + b, tX):                         # right
            pos = shoot(field, startX, startY, x, 0 + b)
            if pos[0] > -1:
                c += 1
                print("c=", c, "pos=", pos)
                if c > stopCount:
                    return pos
        if e == c: # no more asteroids to shoot? 
            break
        b += 1

def vaporizeCircle(field, startX, startY, stopCount=-1):
    x = startX
    y = startY
    d = 1
    c = 0
    ...
    while d < len(field) * 2:  # cycle around the start XY
        y -= 1                                  # 1 up
        pos = shoot(field, startX, startY, x, y)  
        if pos[0] > -1:
                c += 1
                print("c=", c, "pos=", pos)
                if c > stopCount:
...
if len(sys.argv) > 1 and sys.argv[1] == "test":
    ...
    while i < len(tests):
        ...
        lastAsteroid = vaporize(field3, tests[i+1], tests[i+2], 200)
        print(lastAsteroid)
        printField(field3)
